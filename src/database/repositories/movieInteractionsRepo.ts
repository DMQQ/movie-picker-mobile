...
export function createMovieInteractionsRepo(db: SQLiteDatabase) {
  return {
    ...
    async remove(movieId: number, movieType: MovieType, interactionType: InteractionType): Promise<void> {
      await db.runAsync(
        "DELETE FROM movie_interactions WHERE movie_id = ? AND movie_type = ? AND interaction_type = ?",
        [movieId, movieType, interactionType]
      );
    },

    async getByInteractionType(interactionType: InteractionType): Promise<MovieInteraction[]> {
      return db.getAllAsync<MovieInteraction>(
        "SELECT * FROM movie_interactions WHERE interaction_type = ? ORDER BY created_at DESC",
        [interactionType]
      );
    },
    ...
    async getAll(): Promise<MovieInteraction[]> {
      return db.getAllAsync<MovieInteraction>(
        "SELECT * FROM movie_interactions ORDER BY created_at DESC"
      );
    },
    ...
    async getIds(interactionType: InteractionType): Promise<number[]> {
      const results = await db.getAllAsync<{ movie_id: number }>(
        "SELECT movie_id FROM movie_interactions WHERE interaction_type = ?",
        [interactionType]
      );
    ...
    async clearByInteractionType(interactionType: InteractionType): Promise<void> {
      await db.runAsync(
        "DELETE FROM movie_interactions WHERE interaction_type = ?",
        [interactionType]
      );
    },
