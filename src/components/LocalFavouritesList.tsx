    ...
    <FlatList
      ...
      ListFooterComponent={
        <View style={styles.footerContainer}>
          <Pressable onPress={() => router.push("/group/super-liked")} style={styles.footerCard}>
          ...
          <Pressable onPress={() => router.push("/group/blocked")} style={styles.footerCard}>
            <View style={{ borderRadius: 10, overflow: "hidden", position: "relative" }}>
              <ImageBackground
                blurRadius={20}
                style={styles.cardBg}
      ...
      renderItem={({ item }) => (
        <Link
          ...
          href={{ pathname: "/group/[id]", params: { id: item.id, group: JSON.stringify(item) } }}
        >
          <Link.Trigger>
            <View style={{ borderRadius: 10, overflow: "hidden", position: "relative" }}>
              <ImageBackground
                blurRadius={20}
                style={styles.cardBg}
                source={{ uri: "https://image.tmdb.org/t/p/w500" + item?.movies[0]?.imageUrl }}
              >
                ...
                <View style={styles.thumbnailGrid}>
                  {item.movies.slice(0, 4).map((m: any) => (
                    <Thumbnail key={m.id} path={m.imageUrl} size={200} container={styles.thumbnail} />
                  ))}
                </View>
              </ImageBackground>
              <View style={styles.labelRow}>
